    ...
    def initialize_vision_modules(self, model_args):
        ...
        if self.axt2_enable:
            ...
            if getattr(self, "udml_fusion", None) is None:
                udml_hidden_size = (
                    getattr(self.mm_projector, "medgemma_input_dim", self.config.mm_hidden_size)
                    if getattr(self.config, "medgemma_adapter", False)
                    and getattr(self, "mm_projector", None) is not None
                    else self.config.mm_hidden_size
                )
        ...
        if model_args.pretrain_vision_model is not None:
            ...
            logger.info("[ViT Sagittal] Loaded pretrained weights with strict=True")

            if self.axt2_enable:
                self.vision_tower_ax.vision_tower.load_state_dict(vision_model_weights, strict=True)
                logger.info("[ViT Axial] Loaded pretrained weights with strict=True")
        ...
        if model_args.pretrain_mm_mlp_adapter is not None and not getattr(self.config, "medgemma_adapter", False):
            ...
            for k, v in proj_weights.items():
                if k in model_sd and model_sd[k].shape == v.shape:
                    ...
                    skipped.append(k)
            if skipped:
                logger.warning("[Projector] Skipped {} shape-mismatched keys (random init): {}", len(skipped), skipped)
            if compatible:
                self.mm_projector.load_state_dict(compatible, strict=False)
                logger.info("[Projector] Loaded {} compatible weights", len(compatible))
        elif getattr(self.config, "medgemma_adapter", False):
            logger.info("[Projector] MedGemma adapter mode — pretrained weights will be injected by train.py")
...
class LamedMetaForCausalLM(ABC):
    ...
    def encode_all_images(self, images_sag, images_ax=None, sag_noise_variance=None, ax_noise_variance=None):
        ...
        feat_sag = model.get_vision_tower()(images_sag)

        if model.axt2_enable and images_ax is not None:
            feat_ax = model.vision_tower_ax(images_ax)

            if getattr(model.mm_projector, "medgemma_adapter", False):
                sag_med = model.mm_projector.encode_medgemma_inputs(feat_sag)
                ax_med = model.mm_projector.encode_medgemma_inputs(feat_ax)
                ...
                image_features = model.mm_projector.project_medgemma_inputs(final_feat)
        ...
        if model_args.pretrain_mm_mlp_adapter:
            ...
            if "model.embed_tokens.weight" in mm_projector_weights:
                ...
                if input_embeddings.shape == embed_tokens_weight.shape:
                    ...
                    logger.warning(
                        "[Vision Tokenizer] Skipping embed_tokens from pretrained adapter "
                        "(shape {} != {}). Using backbone embeddings instead.",
