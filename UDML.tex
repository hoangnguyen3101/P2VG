...
For a training sample, let \(x_s\) and \(x_a\) denote the sagittal and axial MRI inputs. The two 3D vision encoders produce patch-token features
\[
    H_s = E_s(x_s) \in \mathbb{R}^{B \times N \times C_v},
    \qquad
    H_a = E_a(x_a) \in \mathbb{R}^{B \times N \times C_v},
\]
where \(C_v=768\). In MedGemma adapter mode, each view is spatially pooled and mapped into the native MedGemma visual-projection input space:
\[
    Z_s = A(P(H_s)) \in \mathbb{R}^{B \times N' \times C_m},
    \qquad
    Z_a = A(P(H_a)) \in \mathbb{R}^{B \times N' \times C_m},
\]
where \(P(\cdot)\) denotes spatial pooling, \(A(\cdot)\) is the learnable adapter, \(N'=256\), and \(C_m=1152\). The UDML fusion module operates on \(Z_s\) and \(Z_a\).
...
\[
    \tilde{x}_m
    =
    \operatorname{clip}(x_m + \epsilon_m, 0, 1),
    \qquad
    \epsilon_m \sim \mathcal{N}(0, \eta_m^2 I),
    \qquad
    m \in \{s,a\}.
\]
The sampled noise label is
\[
    \sigma_m \in \{1\} \cup \{\sigma_{\min}, \ldots, \sigma_{\max}\},
\]
and the perturbation standard deviation used in the image space is
\[
    \eta_m = \lambda_{\text{noise}}\sigma_m.
\]
If UDML noise is disabled, or if a sample is selected as clean, then \(\sigma_m=1\).

\subsection{Noise-Aware Variance Estimation}

The token features are pooled into view-level vectors:
\[
...
\[
    ...
    \left(
        g_s(\operatorname{stopgrad}(\bar{z}_s))
    \right)
    + \varepsilon,
    \qquad
    \hat{\sigma}_a
    ...
    \operatorname{softplus}
    \left(
        g_a(\operatorname{stopgrad}(\bar{z}_a))
    \right)
    + \varepsilon.
\]
The stop-gradient operation follows UDML: the variance-estimation loss trains the estimator heads, while preventing this auxiliary objective from directly perturbing the vision encoders.
...
Following the released UDML implementation, inverse-uncertainty target weights are computed as
\[
    \tilde{w}_s
    =
    ...
    \frac{2\hat{\sigma}_s^2}
         {\hat{\sigma}_s^2+\hat{\sigma}_a^2+\varepsilon}.
\]
Therefore, when sagittal uncertainty \(\hat{\sigma}_s\) increases, \(\tilde{w}_s\) decreases; when axial uncertainty \(\hat{\sigma}_a\) increases, \(\tilde{w}_a\) decreases.

\subsection{Shared-Weight Dependency Calculator}

To mimic the shared-weight auxiliary fusion in UDML, a single shared head \(q(\cdot)\) is applied to full and unimodal feature combinations:
\[
    r_{\text{full}} = q([\bar{z}_s;\bar{z}_a]),
\]
\[
    r_s = q([\bar{z}_s;\mathbf{0}]),
    \qquad
    r_a = q([\mathbf{0};\bar{z}_a]).
\]
The parameters of \(q\) are reused for all three branches, analogous to the shared \(fc\_out\) layer in UDML's auxiliary fusion module.

The dependency scores are computed from the magnitudes of the unimodal auxiliary responses:
\[
    \alpha_s
    =
    \sum_j
...
\]
They are normalized to sum to \(2\), matching the two-modality normalization in UDML:
\[
    w_s
    =
...
\[
    ...
    \right).
\]
Since \(w_s+w_a \approx 2\), the factor \(1/2\) preserves feature scale.

The fused tokens are passed through the pretrained MedGemma visual projection:
\[
    V_f
    =
    W_{\text{MG}}
...
\[
    ...
    \right).
\]
To adapt UDML's unimodal classification losses to report generation, sagittal-only and axial-only visual tokens are also projected through the same MedGemma projector and decoded by the same language model:
\[
    V_s
    =
    W_{\text{MG}}
...
\[
    ...
    \right).
\]
These two losses correspond to UDML's unimodal auxiliary losses, with causal language-model cross entropy replacing classification cross entropy.
...
\[
    ...
    \;+\;
    \text{shared-weight dependency estimation}
    \;+\;
