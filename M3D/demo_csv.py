...
def parse_args(args=None):
    ...
    parser.add_argument("--axt2_enable", type=bool, default=False)
...
def main():
    ...
    # Khởi tạo dual-encoder modules nếu axt2_enable
    if args.axt2_enable:
        m = model.get_model()
        m.axt2_enable = True
        m.config.axt2_enable = True
        if getattr(m, 'vision_tower_ax', None) is None:
            m.vision_tower_ax = build_vision_tower(m.config)
        if getattr(m, 'modal_fusion', None) is None:
            m.modal_fusion = nn.Sequential(
                nn.Linear(m.config.mm_hidden_size * 2, m.config.mm_hidden_size),
                nn.GELU()
            )
