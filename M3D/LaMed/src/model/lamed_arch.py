...
class LamedMetaModel:
    ...
    def initialize_vision_modules(self, model_args):
        ...
        if model_args.pretrain_vision_model is not None:
            ...
            if self.axt2_enable:
                # Weight inflation/copy for Axial branch
    ...
    def initialize_seg_modules(self, model_args):
        ...
        self.axt2_enable = getattr(config, "axt2_enable", False)
        if self.axt2_enable:
            self.vision_tower_ax = build_vision_tower(config)
            self.modal_fusion = nn.Sequential(
...
class LamedMetaForCausalLM(ABC):
    ...
    def encode_all_images(self, images_sag, images_ax=None):
        ...
        if model.axt2_enable and images_ax is not None:
            ...
            # 3. Fuse features before projector
            # Concatenate along feature dimension
            fused_feat = torch.cat([feat_sag, feat_ax], dim=-1) # [B, 2048, 1536]
            fused_feat = model.modal_fusion(fused_feat) # [B, 2048, 768]
            final_feat = fused_feat
        else:
            final_feat = feat_sag
            
        # 4. Project once
        image_features = model.mm_projector(final_feat) # [B, 256, 3072]
        return image_features
