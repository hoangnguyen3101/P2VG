...
from LaMed.src.dataset.multi_dataset import(
    SpineCapDataset,
    SpineCapSegDataset
)
from LaMed.src.model.language_model import LamedLlamaForCausalLM, LamedPhi3ForCausalLM, LamedGemma3ForCausalLM
...
class ModelArguments:
    ...
    model_type: Optional[str] = field(default=None, metadata={"help": "llama2, phi3, gemma3"})
    ...
    pretrain_seg_module: str = field(
        default=None, metadata={"help": "Pretrained segvol model."}
    )
...
class DataArguments:
    ...
    seg_data_path: str = field(
        default="./Data/data/M3D_Seg_npy/",
        metadata={"help": "Path to segmentation data."},
    )
    refseg_data_train_path: str = field(
        default="./Data/data/M3D_RefSeg_npy/M3D_RefSeg.csv",
        metadata={"help": "Path to refering segmentation data."},
    )
    refseg_data_test_path: str = field(
        default="./Data/data/M3D_RefSeg_npy/M3D_RefSeg_test.csv",
        metadata={"help": "Path to refering segmentation data."},
    )
...
class DataCollator:
    ...
    def __call__(self, batch: list) -> dict:
        if self.seg_enable:
            images, images_ax, input_ids, labels, attention_mask, segs = tuple(
                [b[key] for b in batch]
                for key in ("image", "image_ax", "input_id","label", "attention_mask", "seg")
            )

            images = torch.cat([_.unsqueeze(0) for _ in images], dim=0)
            images_ax = torch.cat([_.unsqueeze(0) for _ in images_ax], dim=0)
            ...
            images, images_ax, input_ids, labels, attention_mask = tuple(
                [b[key] for b in batch]
...
def main():
    ...
    if model_args.vision_tower is not None:
        ...
        elif "gemma3" in model_args.model_type:
            model = LamedGemma3ForCausalLM.from_pretrained(
                model_args.model_name_or_path, cache_dir=training_args.cache_dir
            )
        else:
    ...
    train_dataset = torch.utils.data.ConcatDataset(
        [
            train_dataset,
            # CTRate_Dataset(data_args, tokenizer, target_shape=(256, 256, 32)),
            # SpineCapDataset(
            #     data_args, tokenizer, target_shape=(256, 256, 32), mode="train"
            # ),
        ]
    )
    ...
    eval_dataset = torch.utils.data.ConcatDataset(
        [
            # AMOSVQADataset(
            #     data_args, tokenizer, target_shape=(256, 256, 32), mode="val"
            # ),
            eval_dataset,
        ]
    )
